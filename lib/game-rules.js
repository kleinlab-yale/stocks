...
export function allocateFifoSale(lots, requestedSharesMicros, priceCents) {
  ...
  for (const lot of lots) {
    if (needed <= 0) break;
    const available = Number(lot.remainingSharesMicros);
    if (available <= 0) continue;
    const sharesMicros = Math.min(available, needed);
    const lotBasis = Number(lot.remainingBasisCents);
...
export function applyPendingWashLosses(
  ...
  for (const loss of washLosses) {
    if (needed <= 0) break;
    const availableShares = Number(loss.remainingSharesMicros);
    const availableLoss = Number(loss.remainingLossCents);
